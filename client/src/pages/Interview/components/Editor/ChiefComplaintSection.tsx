...
const ChiefComplaintSection: React.FC<ChiefComplaintSectionProps> = ({ form }) => {
  ...
  const handleSmartAnalyze = async (text: string) => {
    ...
    try {
      ...
      if (res.success && payload) {
        ...
        // 识别成功后清空智能识别区的输入
        setSmartInputValue('');
        logger.info('[ChiefComplaintSection] 主诉解析完成，已清空智能识别区输入', payload);
      }
    } catch (error) {
      logger.error(error);
  ...
  useEffect(() => {
    ...
    if (ccText !== autoGenerated && ccText !== lastAutoRef.current) {
      ...
      logger.info('[ChiefComplaintSection] 检测到用户手动修改主诉文本', { text: ccText, autoGenerated });
    }
  ...
  useEffect(() => {
    ...
    if (hasAll && structuredChanged && !userModifiedTextRef.current && (!current || current === lastAutoRef.current)) {
      ...
      logger.info('[ChiefComplaintSection] 自动生成主诉', next);
    }
