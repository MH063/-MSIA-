/**
 * 系统回顾(Review of Systems)安全防护模块
 * 提供数据验证、XSS防护、数据完整性校验等安全功能
 */

/**
 * 硬编码的系统回顾症状配置
 * 不依赖数据库，确保基础数据的完整性和不可篡改性
 */
 ...
 * 所有有效的系统key集合
 */
export const VALID_SYSTEM_KEYS = new Set(ROS_SYSTEMS_CONFIG.map(s => s.key));

/**
 * 所有有效的症状key集合
 */
export const VALID_SYMPTOM_KEYS = new Set(
...
export const SYSTEM_TO_SYMPTOMS_MAP: Record<string, Set<string>> = Object.fromEntries(
  ROS_SYSTEMS_CONFIG.map(s => [s.key, new Set(s.symptoms.map(sym => sym.key))])
);

/**
 * 症状key到显示名称的映射
 */
export const SYMPTOM_KEY_TO_NAME_MAP: Record<string, string> = Object.fromEntries(
  ROS_SYSTEMS_CONFIG.flatMap(s => s.symptoms.map(sym => [sym.key, sym.name]))
);

/**
 * 症状名称到key的映射
 */
export const SYMPTOM_NAME_TO_KEY_MAP: Record<string, string> = Object.fromEntries(
  ROS_SYSTEMS_CONFIG.flatMap(s => s.symptoms.map(sym => [sym.name, sym.key]))
);
 ...
 * @returns 验证结果
 */
export function validateDetails(details: string): { valid: boolean; error?: string; sanitized?: string } {
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    if (typeof systemData.details === 'string') {
      ...
      if (detailsResult.valid && detailsResult.sanitized) {
        ...
        errors.push(`系统[${key}]详情验证失败: ${detailsResult.error}`);
      }
    }
 ...
 * @returns 系统回顾选项配置
 */
export function getRosOptions(): typeof ROS_SYSTEMS_CONFIG {
  return ROS_SYSTEMS_CONFIG;
}
 ...
 * @returns 症状选项数组
 */
export function getSymptomOptionsBySystem(systemKey: string): { label: string; value: string }[] {
